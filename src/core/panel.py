...
from core import textInput, tools
...
for colorLabel in tools.COLOR_LIST: FORMAT_TAGS["<%s>" % colorLabel] = (tools.getColor, colorLabel)
...
class Panel():
  ...
  def addfstr(self, y, x, msg):
    ...
    <u>text</u>               underline
    <h>text</h>               highlight
    <[color]>text</[color]>   use color (see tools.getColor() for constants)
